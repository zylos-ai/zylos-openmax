...
5. **实例化 Sub-task（Blueprint 通过后一次性建全部 Step，执行 bot 经发起人确认）**：Blueprint 审批通过后，**必须一次性把全部 Step 实例化成 Task**——**严禁边做边补 / 做一个建一个**（那会让看板只看到零散的当前步骤，丢掉问题全景）。建 Task 时按 Blueprint 的依赖关系设好 `dependsOn`；各 Step 的执行 bot **按技能匹配给出推荐，由发起人确认 / 选择后**再写入 `assigneeId`（可在 Blueprint 审批阶段一并确认各 Step 的 assignee），**不自行拍板**
6. **并行/串行执行（依赖驱动，看板全程可见全景）**：全部 Task 建好后——**无依赖的 Step 并行进入「进行中」(running)**；**有依赖的 Step 停在「待办」(pending) 等待**，其前置 Task `done` 后**自动流转**到「进行中」。因此任务面板 / 看板从一开工就能看到该 Issue 的**完整执行全景**：整条链推进到哪、谁在跑、谁在等、有没有被阻塞(blocked)、待办里卡了什么前置。**关键看板语义：待办列 = 已规划但被依赖挂起的 Step，不是「还没拆出来的步骤」。** Sub-task → Attempt → Agent 执行
...
9. **Blueprint 通过后必须一次性实例化全部 Step 为 Task（强制，依赖驱动）**：复杂任务在 Blueprint 审批通过后，**必须一次性把全部 Step 建成 Task 并设好 `dependsOn`**——**禁止边做边补 / 做一个建一个**。建好后由依赖驱动流转：**无依赖的 Step 并行进入「进行中」，有依赖的停在「待办」等待、前置 done 后自动转「进行中」**。这样看板从一开工就呈现该 Issue 的完整 DAG 全景（在跑 / 在等 / 被阻塞一目了然）；**待办列 = 被依赖挂起的已规划 Step，不是还没拆的步骤**。只实例化当前一步、其余步骤迟迟不进看板 ＝ 把复杂任务做成了 piecemeal，违背设计预期。
...
| Blueprint 通过后只建当前一步 Task、边做边补（piecemeal） | 一次性把全部 Step 建成 Task + 设 dependsOn；无依赖并行、有依赖在待办等待自动流转，看板呈现完整全景 |
