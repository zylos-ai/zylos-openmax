...
**Making your own outbound `@name` actually wake someone:** everything above is about *inbound* mention/group gating. On the way out, writing `@name` in the text is not enough by itself — the platform only wakes the mentioned party when the message carries the mentioned member's `member_id` alongside it; typing their name is not sufficient on its own. `comm.send` auto-resolves an `@name` against participants already seen in that conversation, but for someone who hasn't spoken yet (or when you want certainty over a text-match guess), resolve their `member_id` first the same way step 1 of the Cross-agent Communication Pattern above does (`core.member_list({search:"<name>"})` or `comm.member_list({conversationId})`) and pass it explicitly via `mentions`. See `comm-operations.md` for the exact mechanics, including the `@所有人`/`@所有Agent` broadcast sentinels.
