...
- **Step ③ Guide through the first real task**: following the third question, turn the user's "thing they want to advance recently" into a real delivery. **Advance proactively, do not just wait for the user to give instructions**:
  ...
  - **Return the clickable project link to the user**: as soon as `project.create` succeeds, send the user the clickable link to the new project — build it as `bff_url + /projects?project={project_id}` (easiest: `core.frontend_url {path:"/projects?project=..."}`; see the "Frontend Links" table below). Do not merely state that the project was created — give the user the actual link so they can open it. (Real lesson — issue #135: an onboarding delivery that omits the link leaves the user with only a "working on it" reply and no way to reach the project they just co-created.)
...
When sharing Workspace resource links, build them as **`bff_url + path`**. `bff_url` already carries the deployment's mount prefix (e.g. `/workspace`, the Next.js `basePath` — see `cws-fe/apps/web/src/lib/base-path.ts`; the ingress strips it before forwarding to cws-core), so **do not add `/workspace` yourself** — that is baked into `bff_url`. The `path` column below is the suffix to append.

| Resource | path | Source |
|---|---|---|
| Project list | `/projects` | sidebar.tsx |
| Project detail (selected project) | `/projects?project={project_id}` | projects/page.tsx (fusion page) |
| Issue detail (selected project+Issue) | `/projects?project={project_id}&issue={issue_id}` | projects/page.tsx (fusion page) |
| KB list | `/knowledge` | sidebar.tsx |
...
You can generate the full URL in one step with the CLI: `node src/cli/core.js core.frontend_url '{"path":"/knowledge?kb=xxx&node=yyy"}'` — it prepends `bff_url` for you.
