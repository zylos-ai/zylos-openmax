...
4. **登记 Issue→Task**：在**已确认的项目**下创建 Issue（light）→ Task（认领 / 指派），**再开始执行**——严格遵守「先确认项目 / KB → 再登记 Issue→Task → 再执行」的先后顺序，不要先开干再补登记
5. **指派单一 Agent**：按 agent 描述匹配职责后指派；**不确定该让哪个 bot 执行时，把候选 agent 列给用户、由用户选择**（无匹配专长或用户未指定 → COCO Agent 自己做）
6. **Agent 执行**：该 Agent 独立完成全部工作 → 产出结果
7. **产物归档 & 知识沉淀**：产出 → ArtifactStore；报告沉淀到所选知识库（`/projects/.../research/`）
8. **交付 & 人类验收闭环**：Task 全部 done → `issue.transition` 到 **delivered**，并**主动通知用户请其验收**；**bot 不自行验收 / 归档**。用户**验收通过**（IM 说「验收通过」或看板点验收）→ bot 调 `issue.set_acceptance({accepted:true, source:"im"})` → accepted → **archived（归档）**，必要时沉淀经验；用户**退回** → `issue.set_acceptance({accepted:false, rejectionReason})` → rejected → `issue.transition` reopened → executing 重做。交付到验收之间，issue 停在 **delivered（待验收）**，别当已完成丢着不管
...
8. **交付 & 人类验收闭环**：所有子任务 done → `issue.transition` 到 **delivered** → **主动请用户验收**（bot 不自行验收）→ 用户验收通过 → `issue.set_acceptance({accepted:true})` → **archived（归档）** + 看板同步；退回 → `issue.set_acceptance({accepted:false, rejectionReason})` → rejected → reopened → executing 重做
...
2. **项目/知识库选择必经（简单任务同样适用）**：执行任何「产出 deliverable 的用户任务」（研究 / 分析 / 开发等，**无论简单还是复杂**）前，**必须**让用户确认归属项目 + 产出 KB，不可默默用默认 Inbox/默认 KB。**不要因为任务"简单 / 一个 agent 就能做完"而跳过——简单研究 / 分析报告同样必须先问项目和 KB。** 唯一可跳过：用户已明确指定、纯查询/闲聊、或「内部 bug/问题登记」。
...
6. **人类验收闭环（交付后不自行收尾）**：出 deliverable 的任务，bot 交付（`issue.transition`→delivered）后**必须主动请用户验收，且不得自行 `set_acceptance` / 自行归档**。先后顺序：完成内层流转（attempt→done、task→done）→ issue→delivered → 请人验收。用户**验收通过**才由 bot 调 `issue.set_acceptance({accepted:true, source:"im"})` 收敛到 **archived**；**退回**则 `issue.set_acceptance({accepted:false, rejectionReason})` → reopened → executing 重做。交付到验收之间停在 **delivered（待验收）**，别堆在「已完成」不管——「任务做完不等于结束，人验收通过才归档」。
...
| 交付后 bot 自行 set_acceptance / 自行归档 | delivered 后必须**等人类验收通过**，再由 bot 调 set_acceptance(accepted:true)→archived |
| 交付后把任务堆在「已完成」不管 | delivered=待验收，主动请人验收；任务做完≠结束，人验收通过才归档 |
