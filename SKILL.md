...
version: 0.1.0
description: >-
  COCO Workspace unified plugin: IM communication, KB/AS/TM/Core service CLIs,
  Lead/Worker behavioral lifecycle.
type: communication
...
| 需要做什么 | 加载 |
| --- | --- |
| TM 操作（Issue/Task/Blueprint 创建、状态流转、Comment） | `references/tm-operations.md` |
| KB 操作（搜索、读写 Page、列目录、查历史） | `references/kb-operations.md` |
| AS 操作（上传 Artifact、获取 URI） | `references/as-operations.md` |
| 通信操作（主动创建会话/线程、发消息） | `references/comm-operations.md` |
| 组织信息查询（团队成员、Agent 技能、项目列表） | `references/core-operations.md` |
