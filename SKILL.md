...
description: >-
  ...
  use the explicitly selected owning Project (or ask the human to select one) + confirm KnowledgeBase → register Issue→Task (whoever executes creates it, Issue owner=originator) → execute → it counts as complete only after the owner/originator accepts it,
...
The `proj://<uuid>` and `issue://<uuid>` in a message are canonical references to an existing Project / Issue. The communication bridge resolves them into `<work-references>`; a reference only establishes the context of the current conversation turn, does not start work, does not grant any permission, and does not count as an explicit Project selection for creating a new Issue.
...
- `proj://` is a read/reference context only. It is not itself a task and cannot authorize creating a new Issue. Only a `<project-context>` block generated from validated message metadata counts as the human explicitly choosing the owning Project for a new Issue.
...
An inbound turn contains a `<project-context>` block only when cws-core metadata proves that the human explicitly selected an active Project. Treat it as turn-scoped and immutable; never carry a Project selection from another message or Conversation into the current turn.

- Block present: use its `id` and `name` as context for reads and reasoning without asking the human to select the Project again. Selection still does not mean an Issue must be created: first classify the message, answer chat/questions/status queries directly, and create a new Issue only for an actual new work goal. When creating one, use exactly the block's Project ID plus its `origin-conversation-id` and `origin-message-id`; never substitute Inbox, memory, or another reference.
- Block absent: the human did not select a Project for this message. You may chat, answer questions, and query an existing referenced work object, but **must not call `issue.create` for this message**. If the request needs a new Issue, ask the human to select an active Project and confirm, then wait for a new inbound message that contains the block.
- The Project selection is not Session state. In this single-Session runtime, only the block attached to the current inbound turn is authoritative.
- There is no chat-triggered exception: when the block is absent, do not create a new Issue, including during onboarding or internal bug registration.
...
- **Is a task and does not reference an existing Issue** → first inspect whether `<project-context>` is present. If absent, ask the human to select a Project and stop before Issue creation. If present, the following two things **MUST be done immediately, must not be omitted, and are not exempted just because the task is "simple"**:
  ...
  2. **Use the explicitly selected Project + confirm KnowledgeBase**: `<project-context>` is already the human's Project confirmation, so do not ask for it again. The output KnowledgeBase must still be confirmed before execution; **it is forbidden to silently use the default Inbox/default KB and just start working**.
...
2. **Use or request the Project (explicit selection required before execution)**: when `<project-context>` is present, use that exact Project and do not ask again. When it is absent, ask the user to select an active Project in the composer and confirm; do not create an Issue or default to Inbox while waiting
...
2. **Use the selected Project + confirm KnowledgeBase (before orchestration/execution)**: when `<project-context>` is present, use that exact Project and do not ask the user to confirm it again; still confirm the KnowledgeBase where the output is distilled. When it is absent, ask the user to select an active Project and wait for the confirming message; never infer one from names, references, memory, or Inbox. **Never implicitly create a Project**: creating a new Project is allowed only when the user explicitly requests it and confirms its name. Only after Project selection + KB confirmation may orchestration proceed; this applies during onboarding too
...
- **Step ③ Guide through the first real task**: following the third question, turn the user's "thing they want to advance recently" into a real delivery. **Advance proactively, do not just wait for the user to give instructions**:
  - **Get an explicit Project selection before creating the first-task Issue**: propose a specific first task. If the current turn has no `<project-context>`, ask the user to choose an active Project in the composer and confirm; if they need a new Project, guide them to create it first and then select it in a new message. Only after receiving a turn with `<project-context>` may you call `issue.create` for the first task (set `ownerMemberId` to the user, `leadAgentId` to yourself, and `backlog:false`). Onboarding is not an exception to the no-Project/no-Issue rule.
...
These shortcuts may identify an existing Issue/Project for queries or continuation, but they never authorize creating a new Issue. For a new `issue.create` triggered by an inbound human message, only the current turn's `<project-context>` is authoritative.
...
1. **Validated message context** → use `id`, `origin-conversation-id`, and `origin-message-id` from `<project-context>`; raw names, prose, and `proj://` references do not authorize a new Issue
...
6. **Ask the human** → when Project is unselected, ask them to select an active Project in the composer and wait; never default a new Issue to Inbox

Parameter dependency tree (must be obtained in this order the first time, persist after obtaining):

```
core.me → agentId, orgId
  ├→ project.list → projectId (read/query support only; it cannot replace explicit Project selection for a new Issue)
...
> Distinguish two kinds of actions: pure chat/query can be answered directly and never needs an Issue; any new Issue triggered by an inbound human message requires `<project-context>` and may never default to Inbox. There is no chat-triggered exception.
...
| Treating a Project ID in prose or a `proj://` reference as permission to create a new Issue | Only the current turn's `<project-context>` authorizes a new Issue; otherwise ask the human to select an active Project and confirm |
...
| Starting work first and backfilling Issue/Task registration | First use the explicitly selected Project and confirm KB → then register Issue→Task → then execute, the order cannot be reversed |
