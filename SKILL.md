...
- **为什么**：CLI 已把正确端点路径封装好，尤其 Issue **写路径是 flat `/issues/{id}`，不是 nested `/projects/{pid}/issues/{id}`**（读类列表/创建才嵌套）。手搓 REST 极易猜错路径或字段（flat-vs-nested、PATCH 只接受 `title/description/priority/due_date` 等），这是已知的高频故障源。
- **不确定命令/参数时**：先跑 `node src/cli/<svc>.js`（无参看命令清单），或查 `references/<svc>-operations.md`——**不要凭 REST 惯例猜路径**。
...
5. **实例化 Sub-task（Blueprint 通过后一次性建全部 Step）**：Blueprint 审批通过后，**必须一次性把全部 Step 实例化成 Task**——**严禁边做边补 / 做一个建一个**（那会让看板只看到零散的当前步骤，丢掉问题全景）。建 Task 时按 Blueprint 依赖关系设好 `dependsOn`，并**按"是否有未满足依赖"区分是否带 assignee**：
   - **无依赖、可立即开跑的 Step → 创建时带 `assigneeId`**（指定执行 bot）→ 自动认领进「进行中」，真有 bot 开跑
...
6. **依赖驱动的 bot 自认领推进（状态=真实执行，看板全程可见全景）**：**「进行中」必须对应"真有 bot 在执行"**——后端不会、也不应擅自把待办改成进行中（否则状态是假的）。推进由 **bot 驱动**：
   ...
   - **前置 bot 干完**（attempt→done、task→done）后，**通知下游 bot**（bot-DM，需双向 DM 权限，见跨 agent 沟通模式）「你的前置好了，去认领 task X」→ **下游 bot 自己 `task.claim`**（会校验 `dependsOn`，前置都 done 才放行）→ 成为 assignee + 进「进行中」→ 执行
...
9. **Blueprint 通过后一次性实例化全部 Step（强制）+ 依赖步骤不带 assignee、bot 自认领推进**：审批通过后**一次性把全部 Step 建成 Task 并设 `dependsOn`**，**禁止边做边补**。**有依赖的 Step 创建时不带 `assigneeId` → 停「待办」**；前置完成后由**完成的 bot 通知下游 bot、下游自己 `task.claim`**（认领即校验依赖）进「进行中」——**不是后端自动改状态**（RUNNING 必须 = 真有 bot 在跑，杜绝空转的「进行中」）。只实例化当前一步、其余不进看板 ＝ piecemeal，违背设计预期。（详见复杂任务流程 step 5/6。）
...
| 复杂任务绕过 Blueprint 直接拆 Task 开干（用 light / 无蓝图就实例化）| 复杂（多步/多 agent/有依赖）任务必须 heavy 模式 + 先建 Blueprint 经审批，才能按 Step 实例化 Task |
...
| 期待后端"前置 done 自动把待办变进行中" | 后端不自动改状态；由前置 bot 通知下游、下游 task.claim（校验依赖）才进进行中——RUNNING 必须对应真在执行的 bot |
