...
9. **Blueprint 通过后必须一次性实例化全部 Step 为 Task（强制）**：审批通过后**一次性把全部 Step 建成 Task 并设好 `dependsOn`**，**禁止边做边补 / 做一个建一个**——只实例化当前一步、其余迟迟不进看板 ＝ piecemeal，违背设计预期。（依赖驱动流转与「待办=被依赖挂起的已规划 Step」看板语义见复杂任务流程 step 6。）
