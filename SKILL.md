...
If an owner/admin asks to **connect Feishu, Lark, DingTalk, or WeCom as an inbound chat channel for this Agent** (for example “连接飞书”, “接入 Lark”, “给我钉钉二维码”, “连接企业微信”), load `references/channel-operations.md` and invoke `channel.connect` with the matching channel type. Do not call `conn.list`, ask for App ID/App Secret, or send the user to Agent Manager. Copy the exact server-issued `<message-context conversation-id="..." source-message-id="..."/>` values. The CLI first sends a human confirmation card: ask the user to click its button, never confirm using Agent credentials or claim a QR was sent before confirmation. After the human confirms, the background watcher sends the QR and final Binding status into the conversation. Never expose or restate internal QR/session data.
...
2. **`conn.list {conversationId}`** — which apps/accounts are enabled for **this conversation** (`conversationId` is required — see "Connectors are enabled per conversation" below; for one specific app the user named, use `conn.check {app, conversationId}` instead).
3. **`conn.catalog {app}`** — that app's available actions, each with an `input_schema` describing its parameters.
4. **`conn.invoke {app, action, params, conversationId}`** — run the action (`params` shaped by the `input_schema`). The credential is handled for you; `conn.invoke` auto-routes by the connection's `credential_mode`: **`direct`** = the token is cached locally and the request is assembled from the app's catalog and called from your own egress; **`proxy`** (e.g. a Composio connector) = executed **server-side** with no local token. Either way you just call `conn.invoke {app, action, params}` and never craft a URL or a raw HTTP request.

If an owner authorized **more than one connection of the same app** (e.g. two Gmail accounts), `conn.invoke {app, ...}` returns `needs_selection` (a normal result, not an error) with a `candidates` list instead of silently picking one. Each candidate has a guaranteed-non-empty, unique `label` (its `display_name`, or an app-name + creation-time fallback when unnamed). **Ask the user which one — in the user's own language — referring to each by its `label`, never the `connection_id`** — then retry with `conn.invoke {connectionId, action, params, conversationId}` (the `connectionId` targets that connection directly and skips app-resolution). A non-active connection (needs re-auth / expired / revoked) is rejected up front with an actionable error. See `references/conn-operations.md` → "Multiple connections for one app".

You learn what an app can do by reading its catalog **at call time**, so this one flow covers **any** connected app — you never hardcode or pre-learn a specific provider, and a newly-added connector needs no change here. If `conn.check` reports `not_authorized` for the app the user expects, the connection simply isn't authorized to you yet: **say so and ask the owner to connect/authorize it** — do not invent an alternative mechanism (installing a package, SMTP, scraping, etc.).
...
**Connectors are enabled per conversation.** Each conversation has its own enabled set, and `conn.list` returns only the connectors **enabled for this conversation** — use those freely. A conversation starts with an empty set, so an app being authorized to you does **not** mean it is on in this conversation; do not proactively use a connector that is not in this conversation's `conn.list`.

**Always pass this turn's `conversationId`** to `conn.list`, `conn.check` and `conn.invoke` — copy the `conversation-id` from the turn's `<message-context conversation-id="..." .../>` exactly. It is **mandatory**: without it (or with a malformed one) these verbs refuse with `conversation_context_invalid` and nothing runs. There is no flag or "ops mode" that skips this, so do not look for one; outside a conversation (e.g. a scheduled task with no `<message-context>`) connectors are simply not usable from this CLI — say so rather than working around it.

**Check before you invoke.** When the user asks for a specific app, first run `conn.check {app, conversationId}` (a read-only check; it never executes) and branch on its `state` **before** any `conn.invoke`:

- **`enabled`** — go ahead: `conn.catalog` → `conn.invoke`.
- **`not_enabled`** (case A — authorized to you but switched off in this conversation): do **not** invoke. Ask the user, in their language and in plain words, e.g. 「我可以访问 {app_name}，但当前对话没有启用，要启用吗？」 / "I can access {app_name}, but it isn't enabled in this conversation — want to turn it on?" — and tell them it's the **{app_name}** switch in the connector panel of the message input box. Once they say it's on, re-check and continue. Never enable it on their behalf or work around the switch.
- **`not_authorized`** (case B — not connected to you): send the authorize card with `conn.auth_notice {conversationId, apps:["<slug>"]}` — it shows the user a button that opens that app's connect dialog directly. Alongside it, one short plain sentence is enough (e.g. 「{app} 还没有授权，点卡片去授权；授权完成后会自动在这个对话里启用，回来跟我说一声就行。」 — authorizing from the card auto-enables it for this conversation, so do **not** tell the user to switch it on themselves); do not repeat the card's contents at length. If `conn.auth_notice` fails (e.g. the app isn't in the catalog), fall back to saying plainly that {app} isn't connected yet and pointing them to the **Connections** page (`/connections`). Do not invent another mechanism.
- **`needs_reauth`** (case C — connected but the authorization expired or was revoked): say plainly that the {app_name} authorization has expired and ask them to **re-authorize** it on the Connections page, then try again.

**Earlier results do not outlive the connection.** Data you fetched through a connector earlier in this conversation is not a substitute for a live call: when the user asks about that app again, run `conn.check` first — do not answer from the earlier result. If the state is anything other than `enabled` (revoked, expired, not authorized, or switched off), do **not** repeat, quote, summarize or rely on anything previously fetched through that connector — not even labelled as a snapshot or "as of N minutes ago". Reply only with the matching case A/B/C guidance above. Revoking or switching off a connector means "stop using my data from it", and that includes what you already saw.

**Talk to the user like a person, not a log.** In these replies never show status codes, error codes or internal words (`403`, `409`, `not_enabled_in_conversation`, `conversation_context_invalid`, `connection_id`, "enabled set", "gate", …). If `conn.invoke` itself still returns one of these (e.g. the switch changed mid-conversation), translate it into the same plain sentence as the matching case above. If a check fails because the connector panel is temporarily unavailable (`connector_panel_not_ready` / `connector_enabled_set_unavailable`), say the connectors can't be reached right now and to try again shortly — do not fall back to anything else.

In a **group or thread** conversation there is an additional hard boundary, enforced in code, not just here: a **personal**-scope connector (an individual's own private credential) can never be used outside a direct message. `conn.invoke` rejects it there regardless of the enabled set — do not try to route around it; tell the user in plain words that this is a personal connection and only works in their direct messages with you.
...
### New-Issue Intake (before `issue.create`)

Treat **whether to create an Issue** and **which Project owns it** as two independent decisions. Ask only for a decision the human has not already made explicitly:

| Human's explicit intent | Explicit target Project | What to ask |
|---|---|---|
| Create an Issue | Yes | Nothing; proceed with the confirmed Project |
| Create an Issue | No | Recommend a Project and ask only whether that Project is suitable |
| Do not create an Issue | Irrelevant | Nothing; do not create an Issue or any other Work object |
| Not stated | Yes | Ask only whether to create an Issue in that Project |
...
6. **Dependency-driven advancement: the scheduler notifies the downstream assignee to start (backend state = real execution)**: **RUNNING must correspond to "a bot really executing"** — the backend will not arbitrarily change ASSIGNED to RUNNING. Advancement is driven by **scheduler events + the bot's `task.start`**:
   ...
   - **After the prerequisite task is done, the scheduler (cws-work's System Member) automatically sends a DM to the downstream Task's assignee** "[Scheduler] Task «X»'s dependencies are ready, you can start" (the body names the upstream Task and visibly references the downstream Task) → that assignee **first** calls `task.get` on the referenced downstream Task, reads its authoritative `depends_on` IDs, then calls `task.get` + `comment.list` on each upstream Task to read its completion comments for the output and context, **then** calls **`task.start`** (**the dependency gate is at this step**: it verifies that all `dependsOn` are done before allowing through) → enters **RUNNING** → executes. **Do not rely on hidden message metadata for dependency IDs. No manual DM from the prerequisite bot is needed, and no `task.claim` is needed (the work was already assigned to it in step 5). Since v0.7, `start` is when work begins, the attempt is created, and dependencies are checked.**
...
- **The downstream Worker reads upstream before taking over**: after receiving the scheduler's "dependencies are ready, you can start" DM, **first** call `task.get` on the visibly referenced downstream Task and read its authoritative `depends_on` IDs; then call `task.get` + `comment.list {workType:"task", workId:<upstream taskId>}` on each upstream Task to read its completion comments for the output and context, **then** `task.start`. Do not rely on hidden message metadata for dependency IDs.
...
The following are the **hard actions** for every Issue-backed task from start to finish, not optional suggestions. The new-Issue intake gate itself applies before this lifecycle begins:

1. **Resolve intake before arranging**: before `issue.create`, creation intent must be explicit and the target Project must be explicit or confirmed. Ask only for the missing decision; never treat an Agent recommendation, recent Project, default Inbox, or sole visible Project as confirmation. Once the human chooses Issue-backed work, build the Issue and Blueprint in TM, and create Tasks only after the human accepts the plan. If the human explicitly chooses no Issue, do not create any Work object and do not apply the remaining Issue lifecycle guardrails.
2. **Project/KnowledgeBase selection is explicit (applies to simple Issue-backed tasks too)**: the Project must come from the human's explicit statement/reference or confirmation of an Agent recommendation; do not silently use the default Inbox. The output KB must likewise be explicit or confirmed before Issue-backed execution. **Do not repeat a question the human already answered.** The executing bot must be confirmed by the originator (you may give a recommendation + reasoning based on the agent description), and you must not decide the assignment unilaterally. **Never implicitly create a Project**: Project ownership can only be "choose an existing one" or "create a new one when the user explicitly requests it". If a named Project cannot be resolved unambiguously, ask; do not create a same-named Project as a fallback.
...
> A human request to "register this bug/Issue" already makes Issue creation explicit, but it does not choose a Project. Use an explicit Project from the request or recommend one and wait for confirmation; do not default silently to Inbox.
...
| Creating an Issue before creation intent and Project are explicit/confirmed | Ask only for the missing decision; call `issue.create` only after both are settled |
| Skipping the TM flow after the human chose Issue-backed work | Every Issue-backed requirement must advance via Issue → Blueprint → Task → Attempt |
...
| Starting Issue-backed work first and backfilling Issue/Task registration | Resolve intake and other missing inputs → register Issue→Task → execute; the order cannot be reversed |
...
| **Connect** | Third-party app connections (OAuth/API): list/status, action discovery + execute, and the app-keyed capability cache under `runtime/connect/` | **any request that touches an external app/account** — send/read mail, post/read messages, calendar, docs/files, trackers, etc.: `conn.check {app, conversationId}` (or `conn.list {conversationId}`) → `conn.catalog {app}` → `conn.invoke {app, action, params, conversationId}` (`conversationId` from `<message-context>` is mandatory; credential handled for you — `conn.invoke` auto-routes by `credential_mode`: `direct` = local token, called from your own egress; `proxy`/Composio = executed server-side, no local token) | `references/conn-operations.md` |
