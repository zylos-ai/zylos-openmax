...
| Blueprint 编排 | 多步骤有依赖 | Issue + Blueprint + Steps → 审批 → Task |
...
1. `issue.create`（mode=light）→ Issue 自动进入 EXECUTING
2. `task.create`（带 assigneeId → 自动 claim + 创建 Attempt）
3. 进入执行
...
4. 审批通过 → Issue 进入 APPROVED → EXECUTING
5. 按 Step 拓扑序创建 Task
...
| 人类反馈 | 理解 | TM 操作 |
|---|---|---|
| "好的"、"可以"、"收到" | accepted | Issue → ACCEPTED → Step 11 |
| "不行"、"不对"、"缺了 X" | rejected | Issue → REJECTED → 重做 |
| "帮我改一下 Y" | rejected + 修改要求 | 同上 |
| 无回复（超过阈值） | pending | 礼貌追问 |

**重做路径**：

1. Issue → REJECTED → REOPENED → EXECUTING（三步，不可跳过 REOPENED）
2. 判断重做范围：
   - "缺了 X" → 新建补充 Task
...
| APPROVED | 审批通过（heavy） | → EXECUTING | — |
| EXECUTING | 执行中 | 创建 Task、监控、交付 | 再次提交审批 |
| DELIVERED | 已交付等待验收 | 等待人类反馈 | 创建新 Task |
| ACCEPTED | 人类验收通过 | 经验沉淀 → ARCHIVED | 重开 |
| REJECTED | 人类拒收 | → REOPENED（不可直接→EXECUTING） | 直接回 EXECUTING |
| REOPENED | 重新打开 | → EXECUTING，创建补充 Task | — |
| ARCHIVED | 归档（终态） | — | — |

Light 模式跳过 DRAFT → PENDING_APPROVAL → APPROVED，直接进入 EXECUTING。
...
| 错误 | 原因 | 正确做法 |
|---|---|---|
| Worker 直接 issue.transition | 违反角色边界 | Issue 状态只由 Lead 流转 |
| 没有 leadAgentId 创建 Issue | 违反角色模型 | Issue 必须有 Lead |
| REJECTED 直接回 EXECUTING | 跳过必经状态 | REJECTED → REOPENED → EXECUTING |
| 跳过 Blueprint 审批 | heavy 模式必须走审批 | submit_for_approval |
