...
- **Connector capability cache + cache-aware call chain, consolidated under `runtime/connect/` (`src/lib/connect-store.js`, `src/cli/conn.js`, `src/comm-bridge.js`, `references/conn-operations.md`, `SKILL.md`).** All connect-side local state now lives under one subtree, split by natural key rather than credential mode: `connections-index.json` (connection → application, both modes), `action-catalog/<applicationId>.json` (application → capability, both modes), and `credentials/<connectionId>.json` (real access_token — **direct/token mode only**; migrated one-time from the old `runtime/credentials/`). New cache-aware CLI verbs give any bot the recommended flow for free without needing to "know" it: `conn.invoke {app, action, params}` resolves the connection for an application from the local index (refreshing from `conn.list` on a miss) then executes — authorization + token injection stay fully server-side (`connection_agents` re-checked every call); `conn.catalog {app|applicationId, refresh?}` returns the action catalog, filling from `conn.app_actions` on a miss / 24h-TTL / explicit refresh; `conn.index {refresh?}` inspects the index. Invalidation is TTL + error-triggered (the app-actions response carries no version/etag yet — a small cws-connect follow-up would enable precise invalidation).

### Changed

- **Proxy-mode connections no longer persist a credential blob (`src/comm-bridge.js`, `src/lib/credential-cache.js`).** The `connection.authorized`/`credential_updated` handlers now acquire + cache a credential **only for direct/token-mode** connections (which hold a real `access_token`); proxy-mode connections are recorded in the index but store nothing locally, since their token is injected server-side by cws-connect at call time and the old proxy cache was write-only (never read back on the request path). Credentials moved to `runtime/connect/credentials/` (one-time best-effort migration from `runtime/credentials/`); `connection.revoked`/`disconnected` now also remove the connection from the local index in addition to clearing any cached credential.
