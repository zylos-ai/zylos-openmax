...
- **comm-bridge: first-boot/catch-up `/sync` no longer wedges forever on an unfetchable message at the HEAD of the backlog (`src/comm-bridge.js`, `src/lib/inbox-ledger.js`, `src/lib/content-fetch-giveup.js`).** Follow-on to #79 / PR#76. PR#76 correctly halts the `/sync` sweep (and leaves the cursor un-advanced) when a message's content can't be fetched, so a **transient** failure is re-pulled in seq order on the next sweep. But if the message at the head of the backlog is **permanently** unfetchable (e.g. a message with an empty body), the sweep halts on it every time: the inbox-ledger gap-detector re-triggers `/sync` from the same cursor every ~10-15s, each retry re-hits the same head and halts again, and the entire backlog behind it is never delivered. A freshly started agent could therefore connect, report online, and sit idle on a backlog it never drained.
  ...
  - The give-up decision is extracted into a small pure, unit-tested helper (`src/lib/content-fetch-giveup.js`); ledger persistence/skip behavior and cross-reload count survival are covered in `src/lib/inbox-ledger.test.js`.
