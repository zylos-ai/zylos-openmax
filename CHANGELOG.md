...
- **F4 readiness capability probe (fail-closed).** The per-conversation connectors endpoint doubles as the panel readiness signal: when it cannot be reached, the enabled set is treated as unresolvable rather than empty, so a gate outage can never silently open access. `conn.list` degrades to an **empty list** (never the unfiltered authorized set) and `conn.invoke` **rejects** — both fail closed — with a named `503` code classifying the failure: `connector_panel_not_ready` when the endpoint is not deployed on this instance (404 / network-level failure, e.g. a half-upgrade) and `connector_enabled_set_unavailable` when it is reachable but errors (5xx). This is the openmax half of the design §5.1 readiness layer.

- **`conn.check {app, conversationId}` — pre-invoke availability check.** Read-only; returns the app's `state` in this conversation (`enabled` / `not_enabled` / `not_authorized` / `needs_reauth`) so the agent answers design §2.4 cases A/B/C in plain language **before** invoking, instead of calling `conn.invoke`, hitting a 403 and explaining it in technical terms (dev1 finding #4). `SKILL.md` now mandates check-before-invoke and plain-language replies with no status/error codes.
- **No stale connector data after revoke/expiry (dev1 finding #7/#12, product decision B).** `SKILL.md` now forbids answering from data fetched earlier in the conversation: a repeat question re-runs `conn.check`, and when the connector is no longer `enabled` the agent must not repeat, quote or summarize previously fetched results (not even as a labelled snapshot) — it replies only with the case A/B/C guidance.

- **`conn.auth_notice {conversationId, apps:[slug...]}` — structured "go authorize" card (design §2.4 case B).** After `conn.check` → `not_authorized`, the agent posts an `AGENT_STRUCTURED` message the cws-fe chat already renders as an authorize card (deep link `/connections?connect=<slug>`), instead of plain text only. Apps are resolved to display name + slug from the application catalog (`GET /connect/applications`, exact match; unknown → `404 unknown_connector`, nothing posted); body schema is the existing FE contract `prototype.connector-auth-notice.v1` (`connector_names[]` / `connector_slugs[]` index-aligned) with a metadata copy and a Chinese `fallback_text`. Dropping the `prototype.` prefix is a coordinated FE+agent follow-up. `SKILL.md` case B now calls it.

### Security

- **`conversationId` is mandatory for `conn.list` / `conn.invoke` (fail-closed, dev1 finding #1).** Previously the per-conversation filter/gate applied only when the caller passed a conversation id — omitting it returned the full authorized list and let `conn.invoke` run any authorized connector, a bypass the agent itself discovered. The CLI is run only from the agent's own shell, so there is no trustworthy way to tell a model call from an ops call (any "ops mode" flag/env would be forgeable — design R4-1), so both verbs (and `conn.check`) now refuse a missing or malformed id with `400 conversation_context_invalid` before any request. Non-conversation code that needs a connector must use a service account, not this CLI. `conn.list` also normalizes the list response so a non-array payload can never fall through unfiltered. **Breaking** for any caller that relied on the id-less behavior.
