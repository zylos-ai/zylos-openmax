...
/**
 * Try to send a text message via comm-bridge's IPC server (WS path).
 * Returns the response on success, null if bridge is unavailable.
 * Throws if bridge is running but WS is disconnected (caller falls back to REST).
 */
async function tryWsBridge(ep, text) {
  let info;
  try { info = JSON.parse(fs.readFileSync(BRIDGE_PATH, 'utf8')); } catch { return null; }
  try { process.kill(info.pid, 0); } catch { return null; }  // process dead

  const conversationId = resolveTargetConversation(ep);
...
async function sendText(_unused, ep, text) {
  // Prefer WS bridge (comm-bridge IPC) — sends via the open WS connection.
  // Falls back to REST if bridge is not running or WS is disconnected.
