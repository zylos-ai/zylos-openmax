...
**鉴权统一**:所有服务共用一把 `api_key`(env `COCO_AUTH_TOKEN`)。当前实现把 api_key 直接当 Bearer 用,**实际上 cws-core spec 要求先换 JWT 再用**(见下文 §鉴权)。
...
| api_key 持久化到 `~/zylos/.env` `COCO_AUTH_TOKEN`(0600 权限) | 同上 |
| workspace_id / org_id / device_id / client_id 落 `config.json` | 同上 |
...
| Env 优先级覆盖 | `COCO_API_URL` / `COCO_KB_URL` / `COCO_AS_URL` / `COCO_WS_URL` / `COCO_AUTH_TOKEN` / `COCO_ORG_ID` 等 |
