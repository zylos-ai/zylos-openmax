...
| 日期 | 2026-05-18 |
| 状态 | 草案 |
| 仓库 | https://git.coco.xyz/coco-workspace/zylos-workspace |
...
zylos-workspace 是 zylos 平台的 **workspace 对接插件**，使 zylos agent 成为 COCO Workspace 的原生成员。

两个核心能力：

| 能力 | 说明 | 类比 |
| --- | --- | --- |
| 原生通信 | 通过 COCO 通信协议（cws-comm）收发消息，与人类和其他 Agent 实时通信 | zylos-lark 对接飞书，zylos-workspace 对接 COCO 原生 IM |
...
```
        ...
        ┌───────────────┼───────────────┐
        │               │               │
   zylos-lark      zylos-workspace    zylos-core
...
- **zylos-workspace**：原生 IM 桥接 + 全部 workspace 服务 CLI + 统一行为技能层
- **zylos-tm**：开发阶段独立迭代的 TM CLI，成熟后合入 zylos-workspace 的 `cli/tm.js`
...
因此 zylos-workspace 是一个大插件，各服务团队向同一个仓库贡献自己负责的 CLI 模块和操作指南。
...
│  │    + Layer 3 索引                           │                  │
│  │                                            │                  │
│  │  skills/tm-operations.md  (L3, 按需加载)    │                  │
│  │  skills/kb-operations.md  (L3, 按需加载)    │                  │
│  │  skills/as-operations.md  (L3, 按需加载)    │                  │
│  │  skills/comm-operations.md(L3, 按需加载)    │                  │
│  │  skills/core-operations.md(L3, 按需加载)    │                  │
│  └────────────────────────────────────────────┘                  │
│                                                                  │
└──────────────────────────────────────────────────────────────────┘
```
...
| skills/*.md | L3 操作指南 | 按需加载：各服务 CLI 的命令列表、参数格式、使用场景 |
...
| 团队 | CLI 代码 | 操作指南 |
| --- | --- | --- |
| TM 团队（我们） | `src/cli/tm.js` | `skills/tm-operations.md` |
| KB 团队 | `src/cli/kb.js` | `skills/kb-operations.md` |
| AS 团队 | `src/cli/as.js` | `skills/as-operations.md` |
| Comm 团队 | `src/cli/comm.js` | `skills/comm-operations.md` |
| Core 团队 | `src/cli/core.js` | `skills/core-operations.md` |
...
SKILL.md 始终加载 (Layer 1+2)
  ...
  └─ Layer 3 操作指南索引
    │
    ▼ (按需，在具体 phase 需要时)
skills/kb-operations.md    ← 上下文组装时
skills/tm-operations.md    ← 创建 Issue/Task 时
skills/as-operations.md    ← 上传产出时
skills/comm-operations.md  ← 主动创建会话时
skills/core-operations.md  ← 查询团队能力时
```

SKILL.md 写的是"Phase 3 上下文组装时，搜索 KB 获取项目知识"，但不写 `kb.search` 的参数细节。Agent 到了那一步按索引加载 `skills/kb-operations.md`，拿到具体命令格式再执行。
...
CLI 命令列表和使用场景详见各 Layer 3 操作指南（`skills/*-operations.md`），此处不重复。
...
```bash
zylos add workspace
# 1. 克隆到 ~/zylos/.claude/skills/workspace/
# 2. npm install
